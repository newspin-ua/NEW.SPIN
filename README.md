...
<style>
  ...
  h1 { font-size: 40px; color: #00c3ff; text-shadow: 0 0 10px #00c3ff; }
  button {
  ...
  .result { margin-top: 40px; font-size: 26px; text-shadow: 0 0 15px #00ffcc; }
</style>
</head>
<body>
<h1>🎁 PROCASE</h1>
...
function openCase() {
  ...
  if (roll <= 0.01) {
    ...
    resultEl.innerHTML = 🎉 Вітаємо! Тобі випав <b>подарунок Telegram</b> на ${gift}⭐!;
    Telegram.WebApp.sendData(gift:${gift});
  } else {
    const stars = Math.floor(Math.random() * 2);
    resultEl.innerHTML = stars > 0
      ? ⭐ Тобі випала <b>${stars}</b> зірка!
      : 😢 Нічого не випало цього разу...;
    Telegram.WebApp.sendData(stars:${stars});
  }
}
</script>
</body>
